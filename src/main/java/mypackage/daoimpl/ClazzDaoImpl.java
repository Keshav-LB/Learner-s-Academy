...
public class ClazzDaoImpl implements ClazzDao {
	...
	public List<Clazz> getAll() {
		...
		SessionFactory sessionFactory = HibernateUtil.buildSessionFactory();
		Session session = sessionFactory.openSession();
		Query query = session.createQuery("select c from mypackage.entities.Clazz c");
		return query.list();
	}
	...
	public Clazz getClazzById(int cid) {
		...
		SessionFactory sessionFactory = HibernateUtil.buildSessionFactory();
		Session session = sessionFactory.openSession();
//		Query query = session.createQuery("select c from mypackage.entities.Clazz c where c.clazz_id = ?1");
//		query.setParameter(1, cid);
		Query query = session.createQuery("select c from mypackage.entities.Clazz c where c.clazz_id = :clazzId");
		query.setParameter("clazzId", cid);
	...
	public Clazz getByName(String className) {
		...
		SessionFactory sessionFactory = HibernateUtil.buildSessionFactory();
        Session session = sessionFactory.openSession();
        Query query = session.createQuery("SELECT c FROM Clazz c WHERE c.className = :className");
