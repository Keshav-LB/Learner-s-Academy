...
public class SubjectDaoImpl implements SubjectDao {
	...
	public void update(Subject subject) throws Exception {
		...
		try {
			tx = session.beginTransaction();
			Subject sub = (Subject) session.get(Subject.class, subject.getSubject_id());
			session.update(sub);
			tx.commit();
		} catch (Exception e) {
			if (tx != null) {
	...
	public List<Subject> getAll() {
		...
		SessionFactory sessionFactory = HibernateUtil.buildSessionFactory();
		Session session = sessionFactory.openSession();
		Query query = session.createQuery("select s from mypackage.entities.Subject s");
		return query.list();
	}
	...
	public Subject getSubjectById(int subjectId) {
		...
		SessionFactory sessionFactory = HibernateUtil.buildSessionFactory();
		Session session = sessionFactory.openSession();
		Query query = session.createQuery("select s from mypackage.entities.Subject s where s.subject_id = ?1");
	...
	public List<Subject> getByClass(Clazz clazz) {
		...
		SessionFactory sessionFactory = HibernateUtil.buildSessionFactory();
        Session session = sessionFactory.openSession();
        Query query = session.createQuery("SELECT s FROM Subject s WHERE s.clazz = :clazz");
